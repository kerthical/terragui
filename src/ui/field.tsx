...
function FieldContent({ className, ...props }: React.ComponentProps<"div">) {
  return <div className={cn("gap-0.5 group/field-content flex flex-1 flex-col leading-snug", className)} data-slot="field-content" {...props} />;
}
...
function FieldError({
  ...
  const content = useMemo(() => {
    ...
    return (
      <ul className="ml-4 flex list-disc flex-col gap-1">
        {uniqueErrors.map((error) => error?.message && <li key={error.message ?? "unknown-error"}>{error.message}</li>)}
      </ul>
    );
  }, [children, errors]);
  ...
  return (
    <div className={cn("text-destructive text-xs/relaxed font-normal", className)} data-slot="field-error" role="alert" {...props}>
      {content}
    </div>
  );
}
