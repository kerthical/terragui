...
function Accordion({ className, ...props }: AccordionPrimitive.Root.Props) {
  return <AccordionPrimitive.Root className={cn("overflow-hidden rounded-md border flex w-full flex-col", className)} data-slot="accordion" {...props} />;
}

function AccordionItem({ className, ...props }: AccordionPrimitive.Item.Props) {
  return <AccordionPrimitive.Item className={cn("data-open:bg-muted/50 not-last:border-b", className)} data-slot="accordion-item" {...props} />;
}
