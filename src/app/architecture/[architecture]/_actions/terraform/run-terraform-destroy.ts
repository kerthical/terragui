...
const runTerraformDestroy = async (params: {
  applyId: string;
  architectureId: string;
  input: MultiProviderApplyInput;
  files: TerraformFileEntry[];
}): Promise<void> => {
  const workdir = await mkdtemp(join(tmpdir(), "terragui-destroy-"));
  ...
  try {
    ...
    await appendLog(params.applyId, "stdout", "Running terraform destroy...");
    await runCommand("terraform", ["destroy", "-auto-approve", "-no-color"], { cwd: workdir, env, applyId: params.applyId });
...
export type StartDestroyResult = { success: true; applyId: string } | { success: false; error: string };

export const startTerraformDestroy = async (architectureId: string, input: MultiProviderApplyInput): Promise<StartDestroyResult> => {
  ...
  const hasTfstate = allFiles.some((file) => /\.tfstate$/i.test(file.path));
  if (!hasTfstate) {
    return { success: false, error: "No Terraform state found for this architecture" };
  }
  ...
  void runTerraformDestroy({ applyId: applyRow.id, architectureId, input: data, files: allFiles }).catch(async (error) => {
